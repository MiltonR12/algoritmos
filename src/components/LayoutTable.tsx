...
function LayoutTable({ children, numColumns, numRows }: Props) {
  ...
  const { orderPriority, setOrderPriority, process, updateTableProcess } =
    useProcessState();

  const orderTableProcess = () => {
    if (orderPriority) {
      const processOrder = orderByPriorityProcess(process);
      const newTable = updateProcessTable(processOrder);
      updateTableProcess(newTable);
    } else {
      const processOrder = orderByStartProcess(process);
      const newTable = updateProcessTable(processOrder);
      updateTableProcess(newTable);
    }
  };
  ...
  return (
    <section className="container mx-auto bg-slate-950 p-5 rounded-xl">
      ...
      <div className="grid grid-cols-2 mb-5">
        ...
        >
          {orderPriority ? "Ordenar por entrada" : "Ordenar por prioridad"}
        </button>
      </div>
